...
class AsyncHttpClientHandlerDiffblueTest {
  ...
  void testChannelInactive_thenThrowPrematureChannelClosureException() throws Exception {
    ...
    ChannelHandlerContext ctx = mock(ChannelHandlerContext.class);
    when(ctx.channel()).thenThrow(new PrematureChannelClosureException("An error occurred"));
   ...
   *
   * <ul>
   *   <li>When {@link Throwable#Throwable(String, Throwable)} with {@code Unexpected I/O exception
   *       on channel {}} and {@link ChannelClosedException#INSTANCE}.
   * </ul>
   *
   * <p>Method under test: {@link AsyncHttpClientHandler#exceptionCaught(ChannelHandlerContext,
   * Throwable)}
  ...
  @Test
  @DisplayName(
      "Test exceptionCaught(ChannelHandlerContext, Throwable); when Throwable(String, Throwable) with 'Unexpected I/O exception on channel {}' and INSTANCE")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({
    "void AsyncHttpClientHandler.exceptionCaught(ChannelHandlerContext, Throwable)"
  })
  void testExceptionCaught_whenThrowableWithUnexpectedIOExceptionOnChannelAndInstance() {
    // Arrange
    AsyncHttpClientConfig config = mock(AsyncHttpClientConfig.class);
    when(config.isUseLaxCookieEncoder()).thenReturn(true);
    when(config.getMaxRedirects()).thenReturn(3);
    ...
    httpHandler.exceptionCaught(
        ctx,
        new Throwable("Unexpected I/O exception on channel {}", ChannelClosedException.INSTANCE));
